...
def sumActivity(oldEventTime, prevIntervalEvent, events):
    ...
    if isinstance(prevIntervalEvent, aw_core.models.Event):
        prevIntervalEventDuration = prevIntervalEvent["duration"].total_seconds()
    else:
        prevIntervalEventDuration = prevIntervalEvent["duration"]

    for event in events[0]:
        if event["duration"] > 1.0:
            if prevIntervalEventDuration > 1.0 and event["id"] != prevIntervalEvent["id"] or prevIntervalEventDuration < 1.0:
